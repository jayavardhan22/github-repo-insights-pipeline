...
def compute_health_score_0_100(
    stars: int, forks: int, open_issues: int, days_since_last_push: Optional[int],
    max_stars: int, max_forks: int,
) -> float:
...
def compute_growth_potential_0_100(
    forks: int, stars: int, age_in_days: int, days_since_last_push: Optional[int], open_issues: int,
) -> float:
...
def compute_maintenance_burden_0_100(
    open_issues: int, stars: int, days_since_last_push: Optional[int], forks: int
) -> float:
...
def compute_quality_score_0_100(
    open_issues: int, stars: int, days_since_last_push: Optional[int], maintenance_burden: float
) -> float:
...
class EnhancedGitHubExtractor:
    ...
    def extract_repo_row(
        self, owner: str, repo: str, company_name: str, company_id: int, company_category: str,
        ...
        try:
            ...
            growth_potential = compute_growth_potential_0_100(forks, stars, max(age_in_days, 1), days_since_last_push, open_issues)
            maintenance_burden = compute_maintenance_burden_0_100(open_issues, stars, days_since_last_push, forks)
            quality_score = compute_quality_score_0_100(open_issues, stars, days_since_last_push, maintenance_burden)
            return {
                "repo_id": int(r.id), "repo_full_name": str(r.full_name), "repo_name": str(r.name),
                "repo_owner": str(r.owner.login), "company_name": company_name, "company_id": int(company_id),
                "category": company_category, "stars": stars, "forks": forks, "open_issues": open_issues,
                ...
                "language": language, "topics": ",".join(topics) if topics else None,
                ...
                "age_in_days": int(age_in_days), "age_in_years": float(age_in_years),
                "tech_category": tech_category, "health_score": None, "repo_maturity": repo_maturity,
                "activity_status": activity_status, "growth_potential": float(growth_potential),
                "popularity_per_year": float(popularity_per_year), "fork_engagement_rate": float(fork_engagement_rate),
                "issue_density": float(issue_density), "maintenance_burden": float(maintenance_burden),
                "quality_score": float(quality_score), "company_tech_diversity": None,
                "company_repo_rank": None, "language_market_share": None,
                "repository_type": repository_type, "industry_relevance": None,
            }
...
def add_company_and_global_calcs(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["health_score"] = df.apply(
        lambda r: float(compute_health_score_0_100(
            int(r["stars"]), int(r["forks"]), int(r["open_issues"]),
            None if pd.isna(r["days_since_last_push"]) else int(r["days_since_last_push"]),
            max_stars, max_forks,
        )), axis=1,
    )
    ...
    def _industry_rel(row) -> int:
        ...
        stars, forks, open_issues = int(row["stars"]), int(row["forks"]), int(row["open_issues"])
        score = 0
...
def main() -> None:
    ...
    schema: List[bigquery.SchemaField] = [
        bigquery.SchemaField("repo_id", "INT64"), bigquery.SchemaField("repo_full_name", "STRING"),
        bigquery.SchemaField("repo_name", "STRING"), bigquery.SchemaField("repo_owner", "STRING"),
        bigquery.SchemaField("company_name", "STRING"), bigquery.SchemaField("company_id", "INT64"),
        bigquery.SchemaField("category", "STRING"), bigquery.SchemaField("stars", "INT64"),
        bigquery.SchemaField("forks", "INT64"), bigquery.SchemaField("open_issues", "INT64"),
        bigquery.SchemaField("created_at", "TIMESTAMP"), bigquery.SchemaField("updated_at", "TIMESTAMP"),
        bigquery.SchemaField("pushed_at", "TIMESTAMP"), bigquery.SchemaField("days_since_last_push", "INT64"),
        bigquery.SchemaField("extracted_at", "TIMESTAMP"), bigquery.SchemaField("language", "STRING"),
        bigquery.SchemaField("topics", "STRING"), bigquery.SchemaField("releases_count", "INT64"),
        bigquery.SchemaField("commits_last_30_days", "INT64"), bigquery.SchemaField("contributors_count", "INT64"),
        bigquery.SchemaField("open_prs", "INT64"), bigquery.SchemaField("closed_prs_last_30_days", "INT64"),
        bigquery.SchemaField("age_in_days", "INT64"), bigquery.SchemaField("age_in_years", "FLOAT64"),
        bigquery.SchemaField("tech_category", "STRING"), bigquery.SchemaField("health_score", "FLOAT64"),
        bigquery.SchemaField("repo_maturity", "STRING"), bigquery.SchemaField("activity_status", "STRING"),
        bigquery.SchemaField("growth_potential", "FLOAT64"), bigquery.SchemaField("popularity_per_year", "FLOAT64"),
        bigquery.SchemaField("fork_engagement_rate", "FLOAT64"), bigquery.SchemaField("issue_density", "FLOAT64"),
        bigquery.SchemaField("maintenance_burden", "FLOAT64"), bigquery.SchemaField("quality_score", "FLOAT64"),
        bigquery.SchemaField("company_tech_diversity", "FLOAT64"), bigquery.SchemaField("company_repo_rank", "INT64"),
        bigquery.SchemaField("language_market_share", "FLOAT64"), bigquery.SchemaField("repository_type", "STRING"),
